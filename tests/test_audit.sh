...
python3 -m src.main init store_audit > /dev/null
...
if python3 -m src.main audit-verify | grep -q "AUDIT OK"; then
     echo -e "${GREEN}PASS: Audit log hợp lệ.${NC}"
else
     echo -e "${RED}FAIL: Audit log hợp lệ mà báo lỗi.${NC}"
     exit 1
fi

# 3. Tấn công: Sửa file log
echo "[2] Attacking audit log..."
LOG_FILE="store_audit/audit.log"
...
rm -rf store_audit dataset_audit