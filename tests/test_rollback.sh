...
rm -rf store_test_rollback
mkdir -p dataset_rb

# 1. Tạo Snapshot 1
echo "Data 1" > dataset_rb/file.txt
python3 -m src.main init store_test_rollback > /dev/null
...
META_PATH="store_test_rollback/snapshots/snapshot_$SNAP2/metadata.json"
...
echo "Running Verify..."
if ! python3 -m src.main verify $SNAP2; then
    echo -e "${GREEN}PASS: Phát hiện Rollback/Chain broken thành công!${NC}"
else
    echo -e "${RED}FAIL: Không phát hiện được Rollback!${NC}"
    exit 1
fi

rm -rf store_test_rollback dataset_rb